...
class liveRead extends EventEmitter {
  ...
  refresh = (string, isTab) => {
    ...
    const out = rl.output
    const colLen = process.stdout.columns - 1
    ...
    if (write) {
      out.write(message, () => {
        ...
        readline.moveCursor(
          out,
          isTab ? this.cursorPos : (this.cursorPos += this.key.length),
          // Handle two line input
          -rolUp - message.split(EOL).filter((e) => e.length > colLen + 1).length * 1
        )
