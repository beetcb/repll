...
class liveRead extends EventEmitter {
  ...
  refresh = (string) => {
    ...
    const outputRows =
      message.split(EOL).reduce((count, e) => count + 1 + Math.floor(e.length / (colLen + 0.01)), 0) - 1
    ...
    if (write) {
      out.write(message, () => {
        ...
        // moveCursor(out, rows + 2, -outputRows)
        // moveCursor(out, 2, 0)
      })
