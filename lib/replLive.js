...
class replLive extends EventEmitter {
      ...
      (() => {
        ...
        const handleKey = input => {
          ...
          switch (i) {
            ...
            case 'return': {
              // Raw mode readline return won't refresh
              // !Note: this will clear rl.line and create a new line, which means
              // pressing ↑ will print what you input last time
              this.cleanWrapper()
              // Line event pretty much similar to readline's
              this.emit('line', rl.history.length + 1)
              break
            }
  ...
  writePlaceholder(placeholder) {
    ...
    else placeholder = this.placeholder
    // Color copied from `ansi-styles` project
    rl.output.write(`\u001B[90m${placeholder}\u001B[39m`)
    readline.moveCursor(rl.output, -placeholder.length)
  }
