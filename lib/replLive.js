...
class replLive extends EventEmitter {
        ...
        const handleKey = input => {
          ...
          switch (i) {
            ...
            case 'backspace': {
              ...
              if (!rl.line)
                this.writePlaceholder(placeholders[this.inputLine - 1])
            ...
            case 'return': {
              ...
              if (!pp) {
                ...
                this.prompt(pp)
                this.writePlaceholder(placeholders[this.inputLine])
                this.lastLine = ''
  ...
  refresh = (string = '', noEOL) => {
    ...
    if (del) {
      ...
      if (rl.line) {
        clearScreenDown(out)
        const offset = rl.line.length - rl.cursor
        if (offset) {
          out.write(rl.line.slice(-offset))
        }
      }
    }
   ...
   * @return {promise} resolves with repll history
   */
  waitClosing = () =>
    new Promise(res => {
